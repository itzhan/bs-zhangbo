<template>
  <div class="table-box">
    <el-form :inline="true" style="margin-bottom:16px"><el-form-item><el-button type="primary" @click="openForm()">新增公告</el-button></el-form-item></el-form>
...
const fetchData = async () => { loading.value = true; try { const { data } = await http.get("/announcements/all", { page: page.value, size: 10 }); tableData.value = data.records; total.value = data.total; } finally { loading.value = false; } };
...
const save = async () => { if (formData.id) { await http.put("/announcements/" + formData.id, formData); } else { await http.post("/announcements", formData); } ElMessage.success("保存成功"); formVisible.value = false; fetchData(); };
const publish = async (row: any) => { await http.put("/announcements/" + row.id + "/publish"); ElMessage.success("发布成功"); fetchData(); };
const del = async (row: any) => { await ElMessageBox.confirm("确认删除？"); await http.delete("/announcements/" + row.id); ElMessage.success("删除成功"); fetchData(); };
onMounted(fetchData);
</script>
