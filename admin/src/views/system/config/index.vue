...
const fetchData = async () => { loading.value = true; try { const data = await http.get('/api/system-configs'); tableData.value = data } finally { loading.value = false } }
const save = async (row: any) => { await http.put('/api/system-configs/' + row.id, row); ElMessage.success('保存成功') }
onMounted(fetchData)
</script>
