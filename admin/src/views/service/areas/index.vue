<template>
  <div class="p-4">
    <el-form :inline="true" class="mb-4"><el-form-item><el-button type="primary" @click="openForm()">新增区域</el-button></el-form-item></el-form>
...
defineOptions({ name: 'ServiceAreas' })
const loading = ref(false), tableData = ref<any[]>([]), page = ref(1), total = ref(0)
const formVisible = ref(false), formData = reactive<any>({})
const fetchData = async () => { loading.value = true; try { const data = await http.get('/api/service-areas/all', { page: page.value, size: 10 }); tableData.value = data.records; total.value = data.total } finally { loading.value = false } }
const openForm = (row?: any) => { Object.assign(formData, row || { id: null, name: '', city: '', district: '', description: '', status: 1 }); formVisible.value = true }
const save = async () => { if (formData.id) { await http.put('/api/service-areas/' + formData.id, formData) } else { await http.post('/api/service-areas', formData) }; ElMessage.success('保存成功'); formVisible.value = false; fetchData() }
const del = async (row: any) => { await ElMessageBox.confirm('确认删除？'); await http.delete('/api/service-areas/' + row.id); ElMessage.success('删除成功'); fetchData() }
onMounted(fetchData)
</script>
