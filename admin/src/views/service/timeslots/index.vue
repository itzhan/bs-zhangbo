<template>
  <div class="table-box">
    <el-form :inline="true" style="margin-bottom:16px"><el-form-item><el-button type="primary" @click="openForm()">新增时段</el-button></el-form-item></el-form>
...
const fetchData = async () => { loading.value = true; try { const { data } = await http.get("/time-slots/all", { page: page.value, size: 10 }); tableData.value = data.records; total.value = data.total; } finally { loading.value = false; } };
...
const save = async () => { if (formData.id) { await http.put("/time-slots/" + formData.id, formData); } else { await http.post("/time-slots", formData); } ElMessage.success("保存成功"); formVisible.value = false; fetchData(); };
const del = async (row: any) => { await ElMessageBox.confirm("确认删除？"); await http.delete("/time-slots/" + row.id); ElMessage.success("删除成功"); fetchData(); };
onMounted(fetchData);
</script>
