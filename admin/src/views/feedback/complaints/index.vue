  ...
  <div class="table-box">
    ...
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="status" label="状态" width="90">
        <template #default="{row}"><el-tag :type="{PENDING:'warning',PROCESSING:'',RESOLVED:'success',CLOSED:'info'}[row.status] || ''">{{ {PENDING:'待处理',PROCESSING:'处理中',RESOLVED:'已解决',CLOSED:'已关闭'}[row.status] || row.status }}</el-tag></template>
      </el-table-column>
...
const fetchData = async () => { loading.value = true; try { const { data } = await http.get("/complaints", { page: page.value, size: 10, status: search.value.status || undefined }); tableData.value = data.records; total.value = data.total; } finally { loading.value = false; } };
const openHandle = (row: any) => { currentId.value = row.id; handleResult.value = ""; handleStatus.value = "RESOLVED"; handleVisible.value = true; };
const doHandle = async () => { await http.put("/complaints/" + currentId.value + "/handle", { handleResult: handleResult.value, status: handleStatus.value }); ElMessage.success("处理成功"); handleVisible.value = false; fetchData(); };
onMounted(fetchData);
</script>
