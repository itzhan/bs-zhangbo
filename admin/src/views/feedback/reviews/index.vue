...
const fetchData = async () => { loading.value = true; try { const { data } = await http.get("/reviews", { page: page.value, size: 10 }); tableData.value = data.records; total.value = data.total; } finally { loading.value = false; } };
const openReply = (row: any) => { currentId.value = row.id; replyContent.value = ""; replyVisible.value = true; };
const doReply = async () => { await http.put("/reviews/" + currentId.value + "/reply", { reply: replyContent.value }); ElMessage.success("回复成功"); replyVisible.value = false; fetchData(); };
const del = async (row: any) => { await ElMessageBox.confirm("确认删除？"); await http.delete("/reviews/" + row.id); ElMessage.success("删除成功"); fetchData(); };
onMounted(fetchData);
</script>
