  ...
  <div class="table-box">
    ...
    <el-dialog v-model="replyVisible" title="回复评价" width="500px">
      ...
      <template #footer
        ...
        ><el-button type="primary" @click="doReply">提交</el-button></template
      >
    </el-dialog>
  </div>
</template>
...
const fetchData = async () => {
  loading.value = true;
  try {
    const { data } = await http.get("/reviews", { page: page.value, size: 10 });
...
const doReply = async () => {
  await http.put("/reviews/" + currentId.value + "/reply", { reply: replyContent.value });
...
const del = async (row: any) => {
  ...
  await http.delete("/reviews/" + row.id);
