    ...
    <el-form :inline="true" style="margin-bottom: 16px">
      <el-form-item label="状态">
        <el-select v-model="search.status" placeholder="全部" clearable style="width: 120px">
          <el-option label="待处理" value="PENDING" /><el-option label="处理中" value="PROCESSING" /><el-option
            label="已解决"
            value="RESOLVED"
    ...
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="status" label="状态" width="90">
        <template #default="{ row }"
          ><el-tag :type="{ PENDING: 'warning', PROCESSING: '', RESOLVED: 'success', CLOSED: 'info' }[row.status] || ''">{{
      ...
      <el-form label-width="80px">
        ...
        <el-form-item label="状态"
          ><el-select v-model="handleStatus"
            ><el-option label="已解决" value="RESOLVED" /><el-option label="已关闭" value="CLOSED" /></el-select
        ></el-form-item>
      </el-form>
      <template #footer
        ><el-button @click="handleVisible = false">取消</el-button
        ><el-button type="primary" @click="doHandle">提交</el-button></template
      >
    </el-dialog>
  </div>
</template>
...
const handleVisible = ref(false),
  handleResult = ref(""),
  handleStatus = ref("RESOLVED"),
  currentId = ref(0);
const fetchData = async () => {
  loading.value = true;
  try {
    const { data } = await http.get("/complaints", { page: page.value, size: 10, status: search.value.status || undefined });
...
const doHandle = async () => {
  await http.put("/complaints/" + currentId.value + "/handle", { handleResult: handleResult.value, status: handleStatus.value });
