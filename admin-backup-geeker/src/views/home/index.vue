...
const overviewList = computed(() => [
  ...
  { label: "待处理订单", value: dashboardData.value.pendingOrders || 0 },
  {
    ...
    value: dashboardData.value.totalOrders
      ? ((((dashboardData.value.statusDistribution || {})["COMPLETED"] || 0) / dashboardData.value.totalOrders) * 100).toFixed(
          1
        ) + "%"
      : "0%"
  }
...
const initCharts = () => {
      ...
      series: [
        {
          type: "pie",
          radius: ["40%", "70%"],
          data: Object.entries(dist).map(([k, v]) => ({ name: statusNameMap[k] || k, value: v })),
  ...
  if (ratingChartRef.value) {
    ...
    chart.setOption({
      ...
      yAxis: { type: "value" },
      series: [
        { data: [1, 2, 3, 4, 5].map(r => dist[r] || 0), type: "bar", itemStyle: { color: "#F59E0B", borderRadius: [4, 4, 0, 0] } }
      ],
      grid: { top: 20, right: 20, bottom: 30, left: 50 }
    });
  }
