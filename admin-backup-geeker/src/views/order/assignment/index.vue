    ...
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="type" label="类型" width="100">
        <template #default="{ row }"
          ><el-tag>{{ row.type === "PICKUP" ? "取件" : "送回" }}</el-tag></template
        >
      </el-table-column>
      <el-table-column prop="status" label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="row.status === 'COMPLETED' ? 'success' : row.status === 'PENDING' ? 'warning' : ''">{{
            { PENDING: "待接单", ACCEPTED: "已接单", COMPLETED: "已完成" }[row.status] || row.status
          }}</el-tag>
        </template>
      </el-table-column>
