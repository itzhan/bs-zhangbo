<template>
  <div class="table-box">
    <el-form :inline="true" style="margin-bottom: 16px"
      ><el-form-item><el-button type="primary" @click="openForm()">新增服务</el-button></el-form-item></el-form
    >
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="status" label="状态" width="80"
        ><template #default="{ row }"
          ><el-tag :type="row.status === 1 ? 'success' : 'info'">{{ row.status === 1 ? "上架" : "下架" }}</el-tag></template
        ></el-table-column
      >
...
const fetchData = async () => {
  loading.value = true;
  try {
    const { data } = await http.get("/service-items/all", { page: page.value, size: 10 });
...
const openForm = (row?: any) => {
  Object.assign(formData, row || { id: null, name: "", category: "", price: 0, unit: "件", description: "", status: 1 });
  formVisible.value = true;
};
const save = async () => {
  if (formData.id) {
    await http.put("/service-items/" + formData.id, formData);
  } else {
    await http.post("/service-items", formData);
  }
...
const del = async (row: any) => {
  ...
  await http.delete("/service-items/" + row.id);
