<template>
  <div class="table-box">
    <el-form :inline="true" style="margin-bottom: 16px"
      ><el-form-item><el-button type="primary" @click="openForm()">新增时段</el-button></el-form-item></el-form
    >
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="status" label="状态" width="80"
        ><template #default="{ row }"
          ><el-tag :type="row.status === 1 ? 'success' : 'info'">{{ row.status === 1 ? "启用" : "禁用" }}</el-tag></template
        ></el-table-column
      >
    ...
    <el-dialog v-model="formVisible" :title="formData.id ? '编辑时段' : '新增时段'" width="500px">
      <el-form :model="formData" label-width="80px">
        <el-form-item label="日期"
          ><el-date-picker v-model="formData.slotDate" type="date" value-format="YYYY-MM-DD"
        /></el-form-item>
...
const fetchData = async () => {
  loading.value = true;
  try {
    const { data } = await http.get("/time-slots/all", { page: page.value, size: 10 });
...
const openForm = (row?: any) => {
  Object.assign(formData, row || { id: null, slotDate: "", startTime: "", endTime: "", capacity: 10, areaId: 1, status: 1 });
  formVisible.value = true;
};
const save = async () => {
  if (formData.id) {
    await http.put("/time-slots/" + formData.id, formData);
  } else {
    await http.post("/time-slots", formData);
  }
...
const del = async (row: any) => {
  ...
  await http.delete("/time-slots/" + row.id);
